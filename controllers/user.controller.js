...
const userLogin = async (req, res) => {
    ...
    try {
      ...
      bcrypt.compare(password, user.password, function (err, result) {
        if (err) {
          return res.status(500).send({ success: false, error: 'An error occured during Password Comparison.' });
        }
      ...
      return res.status(500).send({ success: false, error: 'An error occured while processing the request.' });
    }
  };
